...
public class InventoryWindowTest {
	...
	public void testItemList() {
		...
		actual program, the index of the item we add in this test is 9.*/
		assertEquals("UPC did not match",item.getUPC(),itemlist.get(9).getUPC());
	...
	public void testItemList2() {
		...
		Item item = new Item(12345, "Doritos", 3.00,4, "Food", "3-21-2019");
		itemlist.add(item);
		assertNotEquals("Price is an int",3,itemlist.get(9).getPrice());
	...
	public void testItemList3() {
		...
		itemlist.add(item);
		itemlist.add(item2);
		assertEquals("Not the correct amount of items stored",11, itemlist.size());
	}
